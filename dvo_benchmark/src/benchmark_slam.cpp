...
public:
    ...
    void createReferenceCamera(dvo::visualization::CameraTrajectoryVisualizerInterface *visualizer,
                               const dvo::core::RgbdImage &img, const Eigen::Affine3d &pose);
...
void
BenchmarkNode::renderWhileSwitchAndNotTerminated(dvo::visualization::CameraTrajectoryVisualizerInterface *visualizer,
                                                 const dvo::visualization::Switch &s) {
    ...
    if (cfg_.VisualizationRequired()) {
        ...
        if (load_camera_pose_.value()) {
            if (!cfg_.CameraFile.empty()) {
                ...
                ((dvo::visualization::PclCameraTrajectoryVisualizer *) visualizer)->visualizer().getCameraParameters(3,
                                                                                                                     argv);
...
//  dvo::core::IntrinsicMatrix intrinsics = dvo::core::IntrinsicMatrix::create(517.3, 516.5, 318.6, 255.3);
    ...
    std::string optimized_trajectory_file =
            cfg_.RgbdPairFile.substr(0, cfg_.RgbdPairFile.find_last_of(".")) + "_opt_traj_final.txt";
    ...
    std::cout << "STARTED ONLINE benchmark_slam.cpp" << std::endl;
    sw_online.start();
...
//    std::chrono::high_resolution_clock::time_point t2 = std::chrono::high_resolution_clock::now();
    std::cout << "ENDED BENCHMARK" << std::endl;
    //std::cerr << "input:" << std::endl;
    //std::string tmp;
    //std::cin >> tmp;

//    auto tm_duration = std::chrono::duration_cast<std::chrono::seconds>(t2 - t1).count();
//    std::cout << "chrono time estimation is " << tm_duration << " seconds" << std::endl;
    ...
    std::cout << "EXIT BENCHMARK" << std::endl;
